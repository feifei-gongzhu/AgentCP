...
if ($pythonOk) {
    & $Python -c "import keyring, playwright; import src.sorne.webapp; print('imports ok')"
    $importsOk = $LASTEXITCODE -eq 0
    Report "Python imports" $importsOk "keyring / playwright / Sorne"
...
try {
    ...
    Report "Sorne server" ([bool]$api.ok) "http://127.0.0.1:8765/frontend/"
} catch {
    Report "Sorne server" $false "尚未启动"
}
