...
EXCLUDED_DIRS = {
    ".agentcp-runtime", ".agentcp-work", ".agent-compose", "__pycache__", ".cache",
}
EXCLUDED_NAMES = {
    ".control-plane.lock", ".agentcp-8765.pid", ".agentcp-8765.log",
}
...
def _write_backup_archive(store: ProjectStore, temporary_output: Path) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="agentcp-backup-") as temporary:
...
def verify_backup(path: Path) -> dict[str, Any]:
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        if (
            manifest.get("format") != "agentcp-backup"
        ...
        with tempfile.TemporaryDirectory(prefix="agentcp-verify-") as temporary:
