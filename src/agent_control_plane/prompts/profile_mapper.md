...
12. 对每个有测试价值的目标输出 `priority_target` 和 0-100 的 `target_score`。评分表示渗透测试优先级，不表示漏洞已经成立。
13. 新闻、公告、产品介绍、关于我们等同模板展示内容应聚合成 `routine_network_info`。常规网络信息的 `target_score` 必须为 null，不得填写 0 分或其他分数，也不得进入渗透队列。
14. 无法确定是常规信息还是测试目标时使用 `needs_review`，`target_score` 为 null，不要强行打分。
15. 评分综合 URL 语义、真实功能、技术栈、接口形态、认证边界、副作用和采集状态。`/upload`、管理入口、认证、文件读写、用户/订单/权限接口等通常比纯展示页面优先。
16. `risk_tags` 只写紧凑标签；`score_reason` 只写一句依据；`recommended_tests` 只写建议专项类型，不展开测试步骤。
17. 最终只能输出一个 JSON 对象，不输出 Markdown、解释或思考过程。
