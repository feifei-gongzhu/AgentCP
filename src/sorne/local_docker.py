...
THIRD_PARTY_CLAUDE_SYSTEM_PROMPT = (
    "You are an autonomous, non-interactive Sorne worker. Follow the user "
...
class LocalDockerRuntime:
    """Run one model CLI in an Sorne-owned Docker container.
    ...
    def run(self, prompt: str) -> dict[str, Any]:
        ...
        runtime_root = self.profile.project_path / ".sorne-runtime" / _safe_name(self.profile.member_name)
        ...
        with tempfile.TemporaryDirectory(prefix="sorne-docker-input-") as temporary:
    ...
    def _run_openai_tool_compatibility(
        ...
        *,
        ...
        tools = [
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": (
                        "Submit the final Sorne Worker payload only after all "
        ...
        raise LocalDockerError(
            f"Grok 工具调用超过 {OPENAI_TOOL_MAX_ROUNDS} 轮，未返回 Sorne Worker JSON"
        )
    ...
    def _command(self, input_root: Path, runtime_root: Path) -> tuple[list[str], dict[str, str]]:
        ...
        if self.profile.provider == "claude":
            ...
            if third_party_compatible:
                ...
                # small provider-neutral agent contract; the complete Sorne
        ...
        # property must also be listed in `required`. Sorne's worker protocol is
        ...
        # then passes through Sorne's deterministic Guardian.
...
def _extract_claude_worker_payload(
    ...
    if invalid_kind_sources and not invalid_json_sources:
        raise LocalDockerError(
            f"模型未返回带合法 kind 的 Sorne Worker JSON（{detail}）"
        )
    if assistant_text_count and not assistant_tool_use_count:
        raise LocalDockerError(
            f"模型仅返回叙述文本，未执行工具且未输出 Sorne Worker JSON（{detail}）"
        )
    raise LocalDockerError(f"模型未返回合法的 Sorne Worker JSON（{detail}）")
