...
配置（均未设置时 JEV 完全禁用）：
- ``SORNE_JEV_ENDPOINT``：服务基址（如 https://api.typesafe.ai，
  自动拼接 /v1/systemone）；
- ``SORNE_JEV_API_KEY``：Bearer 凭据；
- ``SORNE_JEV_MODEL``：默认 ``jev-1.13``。
"""
...
class JEVDisabled(RuntimeError):
    """未配置 SORNE_JEV_ENDPOINT：JEV 旁路关闭。"""
...
def jev_configured() -> bool:
    return bool(os.environ.get("SORNE_JEV_ENDPOINT", "").strip())
...
def default_transport(state_text: str, questions: dict[str, Any]) -> dict[str, Any]:
    ...
    base = os.environ.get("SORNE_JEV_ENDPOINT", "").strip()
    if not base:
        raise JEVDisabled("SORNE_JEV_ENDPOINT 未配置，JEV 旁路关闭")
    api_key = os.environ.get("SORNE_JEV_API_KEY", "").strip()
    model = os.environ.get("SORNE_JEV_MODEL", "").strip() or JEV_MODEL_DEFAULT
...
def classify_targets(
    ...
    *,
    ...
    model = str(result.get("model") or os.environ.get("SORNE_JEV_MODEL", "").strip() or JEV_MODEL_DEFAULT)
