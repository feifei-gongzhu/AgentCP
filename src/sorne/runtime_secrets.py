...
KEYCHAIN_SERVICE = "com.sorne.project-secrets.v1"
...
def _keychain_read(vendor: str) -> dict[str, str]:
    ...
    try:
        ...
        raise SecretStoreError("macOS Keychain 中的 Sorne 项目密钥数据已损坏") from exc
    if not isinstance(payload, dict):
        raise SecretStoreError("macOS Keychain 中的 Sorne 项目密钥格式无效")
    return {
...
def _keychain_write(vendor: str, values: dict[str, str]) -> None:
    ...
    result = _run_security([
        ...
        "-a", vendor,
        "-s", KEYCHAIN_SERVICE,
        "-l", f"Sorne project API keys: {vendor}",
        "-X", serialized.hex(),
    ])
    if result.returncode != 0:
...
def _credential_read(vendor: str) -> dict[str, str]:
    ...
    try:
        ...
        raise SecretStoreError("Windows 凭据管理器中的 Sorne 密钥数据已损坏") from exc
    if not isinstance(payload, dict):
        raise SecretStoreError("Windows 凭据管理器中的 Sorne 密钥格式无效")
    return {
        str(member): str(secret)
        for member, secret in payload.items()
        if isinstance(member, str) and isinstance(secret, str) and secret
